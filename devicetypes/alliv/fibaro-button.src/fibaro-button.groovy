...
metadata
{
    ...
    tiles(scale: 2)
    {
        ...
        standardTile("configure", "device.button", width: 2, height: 2, decoration: "flat")
        {
...
def zwaveEvent(physicalgraph.zwave.commands.wakeupv2.WakeUpNotification cmd)
{
    ...
    if (!state.lastBattery || (new Date().time - state.lastBattery) >= 24 * 60 * 60 * 1000)
    {
...
def zwaveEvent(physicalgraph.zwave.commands.centralscenev1.CentralSceneNotification cmd)
{
	...
	def txt = "$device.displayName button $bttn was pushed"
    log.debug txt
    createEvent(name: "button", value: "pushed", data: [buttonNumber: bttn], descriptionText: txt, isStateChange: true)
}
...
def configure()
{
	...
	cmds = execCommands(cmds)
    
    return [createEvent(name: "numberOfButtons", value: 5)] + cmds
}
